...
export default function App() {
  ...
  /* ── SMART CONTRACT CALL (allocate ONLY) ── */
  ...
  return (
    <div>
      <section style={{
        ...
        <div style={{background:T.earth, border:`1px solid ${T.bark}`, borderRadius:"100px", padding:"0.35rem 1.1rem", fontSize:"0.72rem", fontFamily:"'DM Mono',monospace", color:T.wheat, letterSpacing:"0.1em", marginBottom:"2rem", display:"inline-block"}}>AgriStream: Disaster Relief</div>

        <h1 style={{fontFamily:"'DM Serif Display',Georgia,serif", fontSize:"clamp(2.8rem,7vw,5.5rem)", color:"#fff", lineHeight:1.1, maxWidth:"780px", marginBottom:"1.5rem"}}>
          Immediate disaster relief.<br/>
          <em style={{color:T.wheat}}>Zero red tape.</em>
        </h1>

        <p style={{fontSize:"clamp(1rem,2vw,1.2rem)", color:T.mist, maxWidth:"560px", lineHeight:1.7, marginBottom:"3rem"}}>
  ...
  return (
    <div className="fadeUp">
      ...
      <div style={{display:"flex", gap:"1.5rem", marginBottom:"3rem"}}>
        <div style={{flex:1.5, background:T.lime, padding:"2.5rem", borderRadius:T.r, color:T.soil, display:"flex", flexDirection:"column", justifyContent:"space-between"}}>
...
/* ═══════════════════════════════════════════════════════════════
   DEPLOY AID (With Address Validation & Feedback UI)
══════════════════════════════════════════════════════════════════ */
function DeployAid({ onDeploy, setPage }: { addr?: string; onDeploy: any; setPage: (p:Page)=>void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    // Address input sanitization check
    const sanitizedAddress = farmer.trim();
    if (!validateAddress(sanitizedAddress)) {
      setStatus("Error: Invalid Stellar address. Beneficiary key must start with 'G' and be exactly 56 characters.");
      return;
    }
    ...
    const res = await onDeploy(sanitizedAddress, +amount, program, muni);
        ...
        <form onSubmit={submit} style={{display:"flex", flexDirection:"column", gap:"1.5rem"}}>
          ...
          <div>
            ...
            <input 
              style={{
                ...inputStyle, 
                borderColor: farmer && !validateAddress(farmer.trim()) ? T.danger : T.mist 
              }} 
              value={farmer} 
              ...
              disabled={busy} 
            />
            {farmer && !validateAddress(farmer.trim()) && (
              <span style={{fontSize:"0.75rem", color:T.danger, marginTop:"0.25rem", display:"block"}}>Address must start with 'G' and be exactly 56 characters long.</span>
            )}
          </div>

          <div style={{background:T.parch, padding:"1.5rem", borderRadius:T.r, marginTop:"1rem"}}>
            <button type="submit" disabled={busy} style={{ width:"100%", background: busy ? T.mist : T.limeDk, color:"white", border:"none", padding:"1.2rem", borderRadius:T.r, fontSize:"1.1rem", fontWeight:700, cursor:busy ? "not-allowed" : "pointer", display: "flex", justifyContent: "center", alignItems: "center", gap: "10px" }}>
      ...
      <div style={{background:"white", padding:"2rem", borderRadius:T.r, boxShadow:"0 5px 15px rgba(0,0,0,0.03)"}}>
         ...
         <form onSubmit={handleFeedback} style={{display: "flex", flexDirection: "column", gap: "1rem"}}>
            <textarea 
               ...
               placeholder="Report an issue or suggest a feature for the NGO portal..." 
...
function AuditTrail({ records }: { records: Disbursement[] }) {
  return (
    <div className="fadeUp">
      ...
      <div style={{background:"white", borderRadius:T.r, overflow:"hidden"}}>
        <table style={{width:"100%", borderCollapse:"collapse", textAlign:"left"}}>
          <thead style={{background:T.soil, color:T.cream}}>
            <tr>
              <th style={{padding:"1.2rem 1rem"}}>Date / Time</th>
              <th style={{padding:"1.2rem 1rem"}}>Relief Program</th>
              <th style={{padding:"1.2rem 1rem"}}>Beneficiary</th>
          ...
          <tbody>
            {records.map((r) => (
              <tr key={r.id} style={{borderBottom:`1px solid ${T.parch}`}}>
                <td style={{padding:"1rem"}}>{fmt(r.createdAt)}</td>
                <td style={{padding:"1rem"}}>{r.program}<br/><span>{r.municipality}</span></td>
                <td style={{padding:"1rem"}}>{shortAddr(r.farmer)}</td>
                <td style={{padding:"1rem"}}>{r.amount} USDC</td>
                <td style={{padding:"1rem"}}>
                  <a href={`https://stellar.expert/explorer/testnet/tx/${r.txHash}`} target="_blank" rel="noreferrer" style={{color:T.sky}}>View TX ↗</a>
                </td>
              </tr>
            ))}
          </tbody>
